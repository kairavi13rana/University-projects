...
try{
		...
		ArrayList<Map<String,Integer>> list = new ArrayList();
   		...
   		String graphType = request.getParameter("BarGraphWeek"); 
   	...
   	//Make a query
   		String query = "" ;
   	   		...
   	   		query= "select bar,date, WEEKNO, concat(day, ' ', 'WeekNO=',WEEKNO, ' ','period=', period)as new_day_weekNo_period, max(nbr_sold)as nbr_beers_sold from( select b.bill_id, b.bar, b.date, b.drinker, b.day,YEARWEEK(date) as WEEKNO,hour(b.time) as period, time_format(time, '%r')  as periods, sum(t.quantity) as nbr_sold, sum(b.total_price), t.item, t.type,  b.bartender  from bills b inner join transactions t on b.bill_id=t.bill_id "
   	   				+" where b.bar='" +barNAME + "' and b.time between '08:00' AND '24:00' and b.date between '2018-01-01' AND '2018-12-31' "
   	   				 +"group by WEEKNO, period order by WEEKNO,  nbr_sold desc) as abc group by WEEKNO";
   	...
   	//Process the result
   		while (result.next()) { 
   			map=new HashMap<String,Integer>();
   	   		...
   	   		map.put(result.getString("new_day_weekNo_period"),result.getInt("nbr_beers_sold"));
   			list.add(map);
   	    } 
   	    result.close();
        ...
        //Create the chart title according to what the user selected
          ...
          chartTitle = "Busiest period of the week ";
 	  	 legend = "bars_week";
